...
public class MenuState extends State {
	...
	/**
	 * The state in which the menu is shown and some menu functionality is working
	 * 
	 * @param handler
	 */
	public MenuState(Handler handler) {
		...
		uiManager.addObject(
				new UIImageButton(140.00f, 200.00f, 128, 64, Assets.userInterfaceButton, new ClickListener() {
					@Override
					public void onClick() {
						handler.getMouseManager().setUIManager(null); // unset the UIManager so u can't restart the game
	...
	public void tick() {
		 ...
		 * GameStateManager.setState(handler.getGame().gameState);
		 */
		uiManager.tick(); // ticks the UIObjects in the UIManager
	}

	@Override
	public void render(Graphics g) {
		/*
		 * Test Code, draws rectangle at position of the cursor g.setColor(Color.RED);
		 * g.fillRect(handler.getMouseManager().getMouseX(),
		 * handler.getMouseManager().getMouseY(), 8, 8);
		 */

		uiManager.render(g); // Renders the UIObjects in the UIManager
	}
